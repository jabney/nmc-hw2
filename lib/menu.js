 ...
 */
const sizes = [
  'small', 'medium', 'large', 'x-large', 'regular'
]
...
const pizza = [
  ...
  },
  {
    ...
    type: 'pizza',
    desc: 'This pizza is for repeat customers who\'ve been discourtious to our drivers on previous deliveries. We start with the pizza you order and add some additional, non-disclosed bonus toppings free of charge. Bon appétit!',
    price: {
      small: 12.99,
      medium: 14.99,
      large: 16.99,
 ...
 * Delete menu files.
 */
const deleteMenu = async function deleteMenu() {
  try {
...
const createMenu = function createMenu() {
  return Promise.all(menuItems.map(async (item, index) => {
    try {
