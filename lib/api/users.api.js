...
const changeEmail = async function changeEmail(newEmail, token) {
  // Don't proceed if the email hasn't changed.
  if (newEmail === token.userId) {
    return newEmail
  }
...
/**
 * Update part or all of the user record.
 *
 * @type {RequestHandler}
 */
lib.patch = (requestData, response) => {
  userValidators.patch(requestData, async (errors, patchData) => {
    // Check for validation errors.
    if (errors && errors.length) {
      return response(400, { errors })
    }
    ...
    const { tokenId, userData } = patchData
    ...
    // If email is specified in user data, perform update.
    if (userData.email) {
      try {
        ...
        response(500, { error: 'error updating user email'})
      }
    }
...
lib.delete = (requestData, response) => {
  // Vaidate incoming request data.
  userValidators.delete(requestData, async (errors, validationData) => {
    // Check for validation errors.
    if (errors && errors.length) {
      return response(400, { errors })
    }
